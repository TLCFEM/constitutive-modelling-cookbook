\chapter{Geomaterial}
In geotechnical engineering, the compression-positive convention is used in the existing literature. This brings some convenience to geotechnical engineering specific applications but conflicts with the common convention adopted in continuum mechanics (tension positive).

To be consistent with other models, in this chapter the continuum mechanics convention is adopted. Thus the formulations presented here may differ from what readers can find in other literature.
...
A distinctive feature of sand models is that, often a non-associative plastic flow is used and the hardening law is significantly more complex, involving heavy tensor operations, than aforementioned models.
Often the hardening laws are tensor--valued functions, and cannot be simplified to scalar--valued functions.
The numerical complexity is higher.
\subsection{Theory}
\subsubsection{Elastic Constitutive Relation}
Often for sand, the elastic response is nonlinear (to be specific, hyper-elastic).
The shear and bulk moduli often are functions of hydrostatic pressure.
In this model, for simplicity, a linear elastic relation is assumed.
Readers shall be warned this does not reflect the real behaviour of sand.
...
\end{gather*}
Essentially, this is similar to the Armstrong--Frederick type kinematic hardening rule. It shall be noted that $\bn$ is a deviatoric stress, and $\balpha$ stays deviatoric but may not be coaxial with $\bn$. Thus in tensor notation,
\begin{gather}
