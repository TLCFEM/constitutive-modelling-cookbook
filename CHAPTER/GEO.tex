\chapter{Geomaterial}
In geotechnical engineering, the existing literature commonly uses the compression-positive convention. This is convenient for geotechnical-specific applications, but it conflicts with the convention commonly adopted in continuum mechanics, where tension is positive.

To remain consistent with the other models in this book, this chapter adopts the continuum-mechanics convention. As a result, the formulations presented here may differ from those found in the geotechnical literature.
\section{Drucker--Prager Model}\label{sec:drucker_prager}
...
The plastic potential has a form that resembles the yield function.
\begin{gather}
...
More complex laws that allow $\eta$ and $\xi$ to evolve with the development of plasticity are also possible. For simplicity, however, $\eta$ and $\xi$ are assumed to be constants.
...
The above yield function essentially has the form of an ellipse, for example,
\begin{gather*}
...
Note that here we simply take $\alpha=\varepsilon^p_v$ instead of $\alpha=-\varepsilon^p_v$ as in the original literature \cite{SouzaNeto2008}.
\subsection{Formulation}
...
\begin{gather}
\bs=\dfrac{M^2}{M^2+6G\gamma}\bs^\text{trial},\qquad
q=\dfrac{M^2}{M^2+6G\gamma}q^\text{trial}.
\end{gather}
...
\begin{gather}
\bsigma=\bs+p\mb{1}=\dfrac{M^2}{M^2+6G\gamma}\bs^\text{trial}+\left(p^\text{trial}-K\left(\alpha-\alpha_n\right)\right)\mb{1}.
\end{gather}
...
This sand model is a hybrid of several existing models with significant simplifications. Readers should not expect it to give accurate results. Instead, it is intended only to capture the main characteristics of sand. It is presented as a basic framework from which more complex sand models can be developed.

Compared with other sand models, this one should be viewed as a preliminary model, much like the bilinear hardening von Mises model when compared with more advanced metal models.

A distinctive feature of sand models is that non-associative plastic flow is often used, and the hardening law is significantly more complex than in the aforementioned models, often involving heavy tensor operations. The hardening laws are frequently tensor-valued and cannot be simplified to scalar-valued functions. As a result, the numerical complexity is higher.
\subsection{Theory}
\subsubsection{Elastic Constitutive Relation}
For sand, the elastic response is often nonlinear, or more specifically, hyperelastic. The shear and bulk moduli are often functions of hydrostatic pressure. In this model, however, a linear elastic relation is assumed for simplicity. Readers should be aware that this does not reflect the real behaviour of sand.
...
f=\norm{\bs+p\balpha}+mp=\norm{\mb{\eta}}+mp,
\end{gather*}
where $\bs=\dev{\bsigma}$ is the deviatoric stress, $p=\dfrac{1}{3}\tr{\bsigma}$ is the hydrostatic stress, $\balpha$ is the so-called back stress, and $m$ characterises the size of the wedge. For simplicity, $m$ is assumed to be constant in this model.
\subsubsection{Flow Rule}
...
\end{gather*}
\subsubsection{Hardening Law}
The evolution rate of the back stress $\balpha$ is defined in terms of a proper distance measure from the bounding surface. Here, that distance measure is chosen as $h\left(\left(\alpha^b-m\right)\bn-\balpha\right)$, where $h$ is a model constant. Thus,
\begin{gather*}
\dot{\balpha}=\gamma{}h\left(\left(\alpha^b-m\right)\bn-\balpha\right).
\end{gather*}
Essentially, this is similar to the Armstrong--Frederick type kinematic hardening rule. It should be noted that $\bn$ is a deviatoric stress, and $\balpha$ remains deviatoric but may not be coaxial with $\bn$. Thus, in tensor notation,
\begin{gather}
...
To avoid complex derivative calculations, the above four residual equations are not combined further. By defining $\mb{x}=\begin{bmatrix}\gamma&p_{n+1}&\bs_{n+1}&\balpha_{n+1}\end{bmatrix}^\mT$, the local Jacobian can be derived and expressed in compressed matrix form. It should be noted that, for simplicity, the subscript $\left(\cdot\right)_{n+1}$ is dropped.
...
For simplicity, the double contraction is not expressed explicitly; one should always use $\mb{A}^\mT\diag{\mb{c}}\mb{B}$ (in compressed matrix representation) for $\mb{A}:\mb{B}$. The Jacobian has size $1+1+6+6=14$.
...
F=\norm{\bs+p\balpha}+mp=\norm{\beeta}+mp,
\end{gather}
where $\balpha$ is the so-called back stress and $m$ characterises the size of the wedge. For simplicity, $m$ is assumed to be constant in this model.
...
The new state should be computed assuming that there is no plasticity. In that case,
\begin{gather}
...
There are five local residual equations.
...
Before proceeding, note that the stress vector $\bsigma$ can be converted into principal stresses via either Mohr's circle or eigenanalysis.
\begin{gather}
\begin{array}{rr}
\sigma_1\\
\sigma_3
...
The maximum $\sigma_d^\text{max}$ should be recorded, and it should be updated only when the local iteration converges and the converged stress yields a larger $\sigma_d$.
Then, whenever $\sigma_d>\sigma_d^\text{max}$, $f>0$ indicates the development of plasticity, and the elastic modulus should be computed according to \eqsref{eq:duncan_reduction}.
...
Based on the above discussion, one can rewrite \eqsref{eq:duncan_reduction} as
\begin{gather}
...
\State \textbf{Input}: $\bvarepsilon_{n+1}$, $\bvarepsilon_n$, $\bsigma_n$, $\sigma_d^{\text{max}}$
\State \textbf{Output}: $\mb{D}_{n+1}$, $\bsigma_{n+1}$
\State return if \algoref{algo:duncan_model} succeeds
\State assume elastic response, find $\gamma$ using \eqsref{eq:ds_1st_stage_a} and \eqsref{eq:ds_1st_stage_b} as residuals
\State assume plastic response, iterate out \eqsref{eq:ds_two_stage}
\end{algorithmic}
\end{breakablealgorithm}
