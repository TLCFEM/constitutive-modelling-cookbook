\chapter{Tensor Basics}\label{chap:tensor}
In engineering, tensors are often defined in Euclidean spaces and thus belong to the Cartesian tensor type.
The transformation between covariant and contravariant bases, and thus the raising/lowering of indices, is often not emphasised (but implied).

The most likely reason is that orthonormal bases are often used, so covariant and contravariant bases lead to the same coordinates.
For this reason, in this book, only subscripts ($i$, $j$, $k$, $l$, etc.) are used to represent tensor indices, implying that contravariant bases are used. Furthermore, scalars are denoted by normal symbols such as $A$ while vectors and tensors are not distinguished, all denoted by boldface symbols such as $\mb{A}$.

In this chapter, we briefly summarise the main concepts of tensors and tensor algebra.
These concepts are practically useful in 3D constitutive models.
The contents presented here are not complete, but they are sufficient for the purpose of this book.
Readers are referred to monographs on this topic if a systematic introduction is desired.
\section{Notations}
Let $\mb{A}\in\mathbb{R}^2\times\mathbb{R}^2$ denote a second order tensor with contravariant bases $\mb{e}^1$ and $\mb{e}^2$. Then $\mb{A}$ has four components that can be arranged in the following matrix.
...
The operator $\otimes$ stands for the tensor product, which will be introduced later.
In some literature, it is omitted for simplicity, giving
\begin{gather}
...
Rather, the corresponding dot product operator $\cdot$ should be explicitly shown as $\mb{e}^i\cdot\mb{e}^j$ to avoid potential confusion.
Note also that the matrix representation only explicitly shows the four components of $A_{ij}$; the corresponding contravariant bases $\mb{e}^1$ and $\mb{e}^2$ are not shown at all!
This is handy because those bases do not need to be repeated every time, given that they typically do not change within a given context/system/problem.
Tensor notation is often used in tensor operations, making the computation compact and concise and thus easier to follow and understand.
...
\end{gather}
Readers should be familiar with the Voigt notation, as it is widely used due to its simplicity.
...
Note that different vector/matrix representations of a tensor may have different components.
...
\end{bmatrix}\quad\text{Mandel notation}.
\end{gather}
In the above, $\gamma_{xy}=2\varepsilon_{12}=2\varepsilon_{21}$ is commonly known as the engineering shear strain.
Using engineering strains in the Voigt notation is a common practice.
However, due to the discrepancy between stress and strain, the same tensorial operations may behave differently depending on the operands involved.
We further point out that using engineering strain has a physical meaning: vectors $\bsigma$ and $\bvarepsilon$ need to be energy-conjugate pairs such that $\bsigma\cdot\bvarepsilon=\bsigma^\mT\bvarepsilon$ represents the actual energy.
In this sense, the factor \num{2} has to be added to either the shear stress or the shear strain components to ensure energy consistency.
Readers should be aware of this issue, as it often causes confusion, if not mistakes.
...
\end{gather}
with components $C_{ijkl}=A_{ij}B_{kl}$. If both $\mb{A}$ and $\mb{B}$ are symmetric tensors, then $\mb{C}$ has major symmetry
\begin{gather}
C_{ijkl}=C_{klij}
\end{gather}
and minor symmetry
...
Let $\mb{A}=A_i\mb{e}^i\in\mathbb{R}^2$ and $\mb{B}=B_i\mb{e}^i\in\mathbb{R}^2$ be two first order tensors; their column vector representations can be written as
\begin{gather}
...
\end{bmatrix}.
\end{gather}
If tensor $\mb{C}$ is treated as a matrix while tensors $\mb{A}$ and $\mb{B}$ are treated as column vectors, then the tensor product is exactly the outer product between $\mb{A}$ and $\mb{B}$. That is,
...
Now let $\mb{A}=A_{ij}\mb{e}^i\mb{e}^j\in\mathbb{R}^3\times\mathbb{R}^3$ and $\mb{B}=B_{ij}\mb{e}^i\mb{e}^j\in\mathbb{R}^3\times\mathbb{R}^3$ be two symmetric second order \textbf{stress} tensors. Adopting the Voigt notation, their column vector representations can be written as
\begin{gather}
...
\end{bmatrix}.
\end{gather}
It can be seen that \eqsref{eq:tensor_product} still applies, since $C_{ijkl}=A_{ij}B_{kl}$. It is thus convenient to adopt the Voigt notation to perform the tensor product between stress tensors.
...
\end{bmatrix}.
\end{gather}
This \textbf{wrong} expression is the direct result of the scaling factor \num{2} in the shear terms.
They must therefore be scaled back to obtain the correct result.
We will not emphasise this further, but the reader should be clear about this difference and apply the proper scaling vectors/matrices when necessary in compressed matrix representations.
...
\subsubsection{Definition}
We are familiar with the concept of the dot product. Let $\mb{A}=A_i\mb{e}^i$ and $\mb{B}=B_i\mb{e}^i$ be two first order tensors (vectors), then the dot product of $\mb{A}$ and $\mb{B}$ gives a zeroth order tensor (scalar) $C$ as
\begin{gather}
...
The double contraction, also known as the double dot product, is a tensor operation that constructs lower-order tensors from higher-order tensors.
...
\end{split}
\end{gather}
The corresponding derivative is straightforward,
\begin{gather}
    \md{C}=\mb{B}:\md{\mb{A}}+\mb{A}:\md{\mb{B}}.
\end{gather}
In essence, double contraction results in the summation of the element-wise products of every component of two second order tensors.
...
There are different conventions for performing double contraction between tensors of different orders.
In the above example, the last two indices of $\mb{A}$ are contracted with the two indices of $\mb{B}$.
One can, in theory, pick any two indices of $\mb{A}$ to be contracted with $\mb{B}$, although not all choices embody physical meanings.
\subsubsection{Vector/Matrix Representation}
Let $\mb{A}=A_{ij}\mb{e}^i\mb{e}^j\in\mathbb{R}^3\times\mathbb{R}^3$ and $\mb{B}=B_{ij}\mb{e}^i\mb{e}^j\in\mathbb{R}^3\times\mathbb{R}^3$ be two symmetric second order \textbf{stress} tensors. According to \eqsref{eq:tensor_stress} and \eqsref{eq:double_contraction}, their double contraction gives
\begin{multline}
...
\end{gather}
Due to the compact (Voigt) notation, an additional scaling matrix $\mb{S}$ is required to account for the factor \num{2} in shear components if the operation has to be spelled out explicitly in vector/matrix form.
As a result, this scaling matrix will propagate to other operations such as the derivatives shown above.
...
The Mandel notation is constructed on top of an orthonormal basis of second order tensors.
The advantage is obvious: there is no need to handle covariant and contravariant representations.
...
However, it is not widely used in theory, as its physical meaning is less obvious.
It also requires additional translations between force and stress (in equilibrium equations), and between displacement and strain (in kinematics).
For the same reason, it is rarely seen in numerical implementations.
In this book, the Voigt notation is used by default.

Finally, if $\mb{A}$ and $\mb{B}$ represent a stress tensor and a strain tensor, one can easily derive
\begin{gather}
...
It can be confusing, if not frustrating, to realise that for different operands, the scaling matrices $\mb{S}$ are different.
Thus, one should always ask what the operands represent before performing those tensor operations in vector/matrix representations.
This is a vital issue existing at the implementation level but not at the abstraction level.
It can be fully circumvented if, instead of compact vector/matrix representations, one uses full tensor notation, so that second order tensors are represented by 2D arrays and fourth order tensors by 4D arrays, at the cost of performance.

The good news is that the vast majority of the existing constitutive models are formulated in the stress space --- the classic plasticity theory is formulated in the stress space after all.
In practice, we rarely need to apply different scaling matrices in a single constitutive model.
\section{Stress Tensor Norm}
The double contraction between a second order tensor $\mb{A}$ and itself results in a scalar that can be used to characterise the norm of $\mb{A}$. In this sense, the double contraction of second order tensors can be regarded as an equivalent version of the dot product of vectors.

Let $\bsigma\in\mathbb{R}^3\times\mathbb{R}^3$ denote a symmetric stress tensor, and define its Euclidean norm as
\begin{gather}
\begin{split}
...
The derivative of $\norm{\bsigma}$ can be computed by the chain rule.
...
Since $\bsigma:\mathbb{I}=\bsigma$, it is easy to see that
\begin{gather}
    ...
    \end{bmatrix}.
\end{gather}
Note that \eqsref{eq:norm_derivative} is shown in tensor notation.
The corresponding derivative can also be expressed as
\begin{gather}
    \ddfrac{\norm{\bsigma}}{\bsigma}=\bn,
\end{gather}
but it must be emphasised that the double contraction is still implied.

For the column vector representation in the Voigt notation, it is expressed as
...
\end{gather}
In some cases, the chain rule may be applied, resulting in the cancellation of scaling matrices, and one may thus obtain
\begin{gather}
    \ddfrac{\norm{\bsigma}}{\bsigma}=\bn^\mT\qquad\text{all vectors}.
\end{gather}
We thus must emphasise that such a subtle difference should always be kept in mind.

We now proceed to compute the derivative of $\mb{n}$ with respect to $\bsigma$.
\begin{gather}
...
From the above example, we point out that although we typically use tensor notation in analytical expressions and compressed matrix representations in implementations, the two forms may resemble each other while still requiring additional scaling matrices to ensure correctness.
\section{Tensor Function of Stress Tensors}
Some tensor--valued functions of stress tensors are frequently used in the analysis of plasticity. Let $\mb{\beta}=f\left(\bsigma,\mb{\alpha}\right)$ denote a tensor-valued function of the stress tensor $\bsigma$ and other tensors denoted by $\mb{\alpha}$. Let the tensor--valued function $\mb{\gamma}=g\left(\mb{\beta}\right)$ be the normalised version of $\mb{\beta}$, that is
\begin{gather}
...
Depending on the form of $\mb{\beta}$, the compressed vector/matrix representation will differ. This will be discussed in the relevant context.
