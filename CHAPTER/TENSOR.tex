...
We shall further point out that using engineering strain has its physical meaning: $\bsigma$ and $\bvarepsilon$ need to be energy conjugate pairs such that $\bsigma\cdot\bvarepsilon=\bsigma^\mT\bvarepsilon$ has to represent the actual energy.
...
\begin{gather}
    ...
    \end{bmatrix}.
\end{gather}
It shall be noted \eqsref{eq:norm_derivative} is shown in tensor notation. For column vector representation in the Voigt notation, it shall be expressed as
