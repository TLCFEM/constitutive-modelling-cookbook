...
We will not emphasize this but one must be clear about such difference and apply proper scaling vectors/matrices when necessary with compressed matrix representations.
...
There are different conventions to perform double contraction between tensors of different orders.
...
It must be confusing, if not frustrating, to realise that for different operands, the scaling matrices $\mb{S}$ are different.
Thus, one shall always question oneself about what the operands represent before performing those tensor operations in vector/matrix representations.
This is a vital issue existing at the implementation level but not at the abstraction level.
It can be fully circumvented if instead of compact vector/matrix representations, one uses full tensor notations such that second order tensors are represented by 2D arrays and fourth order tensors by 4D arrays, at the cost of performance.
...
\end{gather}
We thus must emphasize that such a subtle difference shall always be kept in mind.
