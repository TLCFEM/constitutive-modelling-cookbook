...
This chapter presents a general framework for plasticity.

A typical elasto-plastic model can be idealised by the frictional device shown in \figref{fig:idealisation}. The device consists of an elastic spring element, whose deformation is fully recoverable when the applied external force becomes zero, and a friction element, which deforms once the deformation exceeds a certain limit and whose deformation cannot be recovered even when the applied external force becomes zero.

Most plastic models are formulated in \textbf{stress space} or, equivalently, are \textbf{strain driven}. The task is to determine the stress response from the strain input.
\begin{figure}[ht]
\centering
...
For the above model, the total strain $\bvarepsilon$ can be decomposed into two parts, namely the recoverable elastic strain $\bvarepsilon^e$ and the unrecoverable plastic strain $\bvarepsilon^p$.
\begin{gather}
...
To determine whether a given stress state is admissible, the yield function is introduced as the criterion. It is often a scalar-valued function of stress $\bsigma$ and some additional internal variables $\bq$, viz.,
\begin{gather}
f=f\left(\bsigma,~\bq\right).
\end{gather}
By convention, it is defined so that all admissible stress states satisfy $f\leqslant0$, while $f>0$ is not allowed. If $f$ is viewed as a surface in stress space, all admissible stress states should lie within the volume bounded by $f$. Thus, the yield function is also called the yield surface in some literature. It may further evolve (change of size, location, etc.) with the development of plasticity. It is sometimes classified into two types: the initial yield surface (when plasticity has not occurred) and the subsequent yield surface (when plasticity has developed and is developing).

The internal variables $\bq$ are called history variables.
\section{Flow Rule}
As plastic strain may evolve during the loading/unloading process, it is necessary to know how it evolves, and the flow rule is defined as follows.
\begin{gather}\label{eq:general_flow_rule}
...
In the context of plasticity, since time $t$ is often not explicitly involved, the dot symbol $\dot{\left(\cdot\right)}$ above a quantity denotes the increment $\dot{\left(\cdot\right)}=\pdfrac{\left(\cdot\right)}{t}\Delta{}t$ for brevity.
\section{Hardening Law}
To allow the yield function to evolve, the internal variables are governed by a function, which is defined as the hardening law.
\begin{gather}
\dot{\bq}=\gamma\mb{h}\left(\bsigma,~\bq\right),
\end{gather}
where $\mb{h}=\mb{h}\left(\bsigma,~\bq\right)$ is another tensor--valued function that indicates the type of hardening.
\section{Consistency Conditions}
Since $f\ngtr0$ and $\gamma\nless0$, only a limited number of admissible situations exist:
\begin{itemize}
...
For a given strain input $\Delta\varepsilon$ and initial conditions, a general algorithm has the following structure.
...
If plasticity must develop, note that $f=0$. With the aid of \eqsref{eq:strain_decomposition}, $\bsigma$ can be equivalently represented by $\bvarepsilon^p$. There are three unknown variables $\gamma$, $\bvarepsilon^p$ and $\bq$ to be solved, subject to the following three equations.
\begin{gather}
\left\{
\begin{array}{l}
...
The above system is often nonlinear by construction and needs to be solved with an iterative method. It can be rewritten in a more compact nonlinear form. Let $\mb{x}=\begin{bmatrix}
...
\begin{gather}\label{eq:local_system}
\mb{R}=\left\{
\begin{array}{l}
...
\end{array}\right.
\end{gather}
The goal is to find the solution $\mb{x}$ to $\mb{R}=\mb{0}$ for a prescribed total strain $\bvarepsilon$. The Newton--Raphson method can be adopted with the help of the Jacobian $\pdfrac{\mb{R}}{\mb{x}}$. Note that partial derivatives are used here because, fundamentally, $\bvarepsilon$ enters the system. It is a variable that is held constant only at the local iteration level.
\subsection{Consistent Tangent Stiffness}\label{sec:consistent_tangent_general}
...
Note that at equilibrium, the local residual is zero, viz., $\mb{R}=\mb{0}$. Taking the full differentiation leads to
...
Although not in closed form, the above procedure provides a universal and highly efficient approach for computing the consistent tangent stiffness, and it can be applied to a wide range of plastic models.
...
Here we present some frequently used tensor quantities defined in three-dimensional space.
\subsection{Spherical Stress}
The spherical stress of a stress tensor $\bsigma$ refers to the portion corresponding to an isotropic hydrostatic pressure $p$,
\begin{gather}
...
\end{gather}
Here, $\mb{1}$ is the unit second order tensor.
\begin{gather}
\mb{1}=\begin{bmatrix}1&1&1&0&0&0\end{bmatrix}^\mT.
\end{gather}
\subsection{Deviatoric Stress}
The remaining portion is often known as the deviatoric stress, denoted by $\mb{s}$.
\begin{gather}
...
Note that, due to the adoption of the Voigt notation, the matrix representation of $\mathbb{I}^\text{dev}$ is \textbf{not} unique.
...
The volumetric strain $\varepsilon^v$ can be defined as
\begin{gather}
...
For a given symmetric tensor $A_{ij}\in\mathbb{R}^3\times\mathbb{R}^3$, it can be explicitly expressed as
\begin{gather}
\mb{A}=
\begin{bmatrix}
A_{11}&A_{12}&A_{31}\\
...
\end{bmatrix}.
\end{gather}
It has three invariants, which are expressed as
...
\end{gather}
The three invariants $I_1$, $I_2$ and $I_3$ are in fact functions of the eigenvalues of $\mb{A}$.
They are invariant under frame transformations since the eigenvalues do not change.
...
The above expressions hold for any symmetric tensor.
They thus apply to deviatoric tensors as well.
The deviatoric stress invariants can then be defined by denoting $\mb{S}=\mb{A}-\dfrac{I_1}{3}\mb{1}$.
\begin{gather}
J_1=\tr{\mb{S}}=0,\\
\begin{split}
...
Readers are encouraged to carry out the above laborious computation to verify the results.
In practice, only a single set of functions needs to be implemented.
When an arbitrary tensor is provided as input, the corresponding invariants should be computed.
When a deviatoric tensor is provided, the deviatoric invariants are computed.

In the following, we further provide some explicit expressions for strain and stress tensors.
Although this may appear confusing, note that \eqsref{eq:tensor_invariant} applies to any given tensor.
...
The spherical part and the deviatoric part are governed by two material parameters, the bulk modulus $K$ and the shear modulus $G$. In tensor notation,
\begin{gather}
p=K\varepsilon^v,\qquad\mb{s}=2G\bvarepsilon^d.
\end{gather}
Note that the latter in component form reads
\begin{gather}
...
\end{gather}
The corresponding compressed matrix representation can be expressed as
...
After some manipulations, the total stress can be expressed in terms of the total strain as follows.
\begin{gather}
\begin{split}
\bsigma&=\mb{s}+p\mb{1}\\
...
For some material models, the Haigh--Westergaard stress space may be used. The Lode angle is often used to define a parameter that alters the yield surface, which may have a special shape on the $\pi$-plane. The Lode angle $\theta$ can be defined as
