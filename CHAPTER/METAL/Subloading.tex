...
\begin{gather}
    f=\norm{\beeta}-F,
\end{gather}
in which $\beeta=\beeta\left(\bsigma,\cdots\right)$ is known as the shifted stress, a function of the stress tensor $\bsigma$ and other internal variables. Here, $F=F\left(q\right)$ characterises the size of this surface and could evolve with the development of plasticity driven by some internal variable $q$.
Within the yield surface $f$, the material is elastic, while plasticity develops once the stress state reaches and exceeds $f$. In that case, the surface itself expands to enclose the current stress state accordingly.
Thus, any admissible state should satisfy the inequality $f\leqslant0$.

The original proposal of the subloading surface model introduces a new internal variable called the normal yield ratio, denoted by $z$ in this work. It effectively scales the yield surface $f$ down by a factor $z$; the new surface is called the subloading surface $f_s$ and is concentric with the yield surface $f$.
\begin{gather}
    f_s=\norm{\beeta}-zF.
\end{gather}
The normal yield ratio $0\leqslant{}z\leqslant{}1$ should be updated so that $f_s=0$ is enforced not only for plastic loading but also for elastic unloading.
With such a construct, any given stress state always lies on the subloading surface, meaning that $f_s=0$ is always satisfied.
...
The evolution rule for the elastic core has undergone several iterations; see, for example, the initial version \cite{Hashiguchi1989}, one variation \cite{Hashiguchi2015}, and the most recent version \cite{Anjiki2019}.
The original formulation further imposes the requirement that the size of the elastic core should be limited to a certain fraction of that of the yield surface.
...
\end{gather}
Here, $0\leqslant{}c_e$ is a constant that controls the evolution rate, $0\leqslant{}z_e\leqslant1$ is a constant that controls the limit size of the elastic core, and $\bn=\pdfrac{f_s}{\bsigma}/\norm{\pdfrac{f_s}{\bsigma}}=\dfrac{\beeta}{\norm{\beeta}}$ is the unit gradient of the subloading surface.
It should be pointed out that the term $z_eF\bn$ is expressed as $z_e\dfrac{\beeta}{z}$ in the original formulation.
...
\begin{gather}\label{eq:original_z}
    \dot{z}=U\dot{q},
\end{gather}
where $U=U\left(z\right)$ is a function that should satisfy certain conditions, and $q$ is the accumulated plastic strain.
...
In scalar form, the Armstrong--Frederick evolution rule \cite{Frederick2007} is equivalent to the following ordinary differential equation (ODE),
\begin{gather}\label{eq:af_ode}
    y'=b\left(a-y\right),
\end{gather}
which can be explicitly integrated to give the solution for the initial condition $y(0)=0$,
\begin{gather}
    y(x)=a\left(1-\exp\left(-bx\right)\right),
\end{gather}
which defines a curve that asymptotically approaches the constant bound $a$.

To employ a non-constant bound, one may simply replace the constant $a$ by a function $a(x)$ in the ODE \eqsref{eq:af_ode}, such that $y'=b\left(a\left(x\right)-y\right)$.
...
\end{gather}
It is also worth noting that the following form
\begin{gather}
...
Here, $y'$ is governed by the difference $a\left(x\right)-y$, while the approaching rate changes from $b$ to $b/a\left(x\right)$.
...
\end{gather}
By comparing with \eqsref{eq:original_core}, one can conclude that \eqsref{eq:polar_decomposition} must be mathematically equivalent to \eqsref{eq:original_core} if $\bd$ is governed by the following evolution rule,
\begin{gather}
    \dot{\bd}=c_e\norm{\dot{\bvarepsilon^p}}\left(z_e\bn-\bd\right).
\end{gather}
If the associative flow rule is adopted, the above expression can be further expressed as
\begin{gather}
...
The evolution rule for $\bc$ shown in \eqsref{eq:original_core} is derived at the limit state where the subloading surface coincides with the limit surface --- a state that would not occur because of its asymptotic nature.
...
\end{gather}
Then the evolution of $\bc$, which characterises a point on the elastic core surface, can be equivalently described by the evolution of $\bd$, which can take a very simple form once the contribution of $F$ is separated,
...
However, such a decomposition separates the directional vector from the scalar magnitude, which allows much simpler evolution rules for both parts, although the vector here is not a unit vector and thus does not possess a fixed unit length.
...
\end{gather}
with $H=H\left(q\right)$ being the bound and $\balpha$ being the normalised back stress (normalised by $H$), which can follow the following evolution rule.
...
However, note that the specific form $\bbeta=b\left(\dot{\bvarepsilon^p}-\norm{\dot{\bvarepsilon^p}}\dfrac{\bbeta}{H}\right)$, which is used by \cite{Anjiki2019,Anjiki2021}, should not be used, as the back stress may go beyond the intended bound ($\norm{\bbeta}>H$) for softening materials; see the discussion around \figref{fig:examples}.
...
Accounting for the proposed decompositions \eqsref{eq:decomposition_core} and \eqsref{eq:decomposition_back}, the above expression can be rewritten as
\begin{gather}
...
\end{figure}
It can also be noted that, since $\norm{\bd}<1$, then $\norm{zF\bd}<zF$. Therefore, the homothetic centre is always inside the subloading surface $f_s$, which itself is inside the yield surface $f$.
\subsection{Loading/Unloading Criterion}\label{sec:correct_loading_criterion}
The conventional Kuhn--Tucker condition cannot be applied in the subloading surface model since, even for elastic unloading, the history variable $z$ must be updated for every load increment/step.
For arbitrary loading directions and magnitudes, it is possible for the normal yield ratio $z$ to decrease first --- the subloading surface shrinks --- and then increase --- the subloading surface expands.
When it expands, plasticity can only be computed correctly with the correct initial value of $z$.
...
In the trial state, by freezing plasticity, viz., keeping $H$, $F$, $\balpha$, and $\bd$ constant, one can conclude that the centre of the subloading surface must lie on the line defined by the centre of the yield surface $\bbeta$ and the similarity centre $\bc=\bbeta+F\bd$.
Then, for a given load increment $\Delta\bs=\bs_{n+1}-\bs_n$, assuming proportional loading, there exists a scalar $r$ such that the corresponding stress state $\bs_n+r\Delta\bs$ minimises $z$. We denote this value by $z^\text{trial}$ since it is computed in the trial state.
...
\end{gather}
where $z^\text{trial}$ should be computed by enforcing the corresponding $f_s^\text{trial}=0$.
...
However, three distinctive cases arise.
\begin{enumerate}
    \item If $0<r<1$, the given load increment $\Delta\bs$ contains both elastic unloading and plastic loading.
          Since the elastic unloading portion ($0\to{}r$) reduces $z$ to a value smaller than the converged $z_n$, the plastic loading portion ($r\to{}1$) should use this minimum $z$ (smaller than $z_n$, not necessarily zero in 3D models), instead of $z_n$, as the starting value to correctly compute plasticity development.
...
\end{enumerate}
Thus, for any given load increment, one can 1) find the corresponding scalar $r$ that minimises $z^\text{trial}$, and 2) use the above criteria to determine whether plasticity occurs and, if so, correct the starting value of $z$ if necessary.
...
For the von Mises criterion, $\bs$ takes the deviatoric part of the stress tensor, $\bs=\dev{\bsigma}$, then
\begin{gather}
    \beeta=\dev{\bsigma}-H\balpha+\left(z-1\right)F\bd.
\end{gather}
The subloading surface can then be expressed as follows by replacing $F$ with $\sqrt{\dfrac{2}{3}}\sigma^y$.
...
The evolution of $\sigma^y$ depends on $q$. Here, we adopt a general form that combines linear hardening and nonlinear saturation.
...
In this particular case, it can be written as
\begin{gather}\label{eq:normalised_back_rule}
    \dot{\balpha}=b\left(\sqrt{\dfrac{2}{3}}\bn-\balpha\right)\gamma.
\end{gather}
It must be pointed out that, strictly speaking, \eqsref{eq:normalised_back_rule} should be expressed as \cite{Simo1998}
\begin{gather}
...
Note that the constant $\sqrt{\dfrac{2}{3}}$ is added to the evolution rules for $\alpha$ and $d$ in the uniaxial version to ensure that the same set of parameters used for both models yields the same results.
They can be removed if such consistency is not required.
\begin{table}[htb]
...
It allows a simple and concise formulation that is beneficial for both theoretical interpretation and numerical implementation.
\subsection{Formulation}
\subsubsection{Trial State}
Similar to the conventional return mapping algorithm, the trial state is computed by assuming that the given load increment is fully elastic. One can then compute the trial stress $\bsigma^\text{trial}$ via the constitutive law.
\begin{gather}
    \bsigma^\text{trial}=\bsigma_n+\mb{D}_e:\left(\bvarepsilon_{n+1}-\bvarepsilon_n\right)=\bsigma_n+\mb{D}_e:\Delta\bvarepsilon.
\end{gather}
The increment of the deviatoric part, $\bs^\text{trial}=\dev{\bsigma^\text{trial}}$, can be computed accordingly.
\begin{gather}
...
\end{gather}
Noting that $\norm{\bd_n}<\sqrt{\dfrac{2}{3}}z_e\leqslant\sqrt{\dfrac{2}{3}}$, we have $\dfrac{2}{3}-\bd_n:\bd_n>0$; thus, the denominator is positive.
For the same reason, $\sqrt{\left(\bar{\beeta}:\bd_n\right)^2+\left(\bar{\beeta}:\bar{\beeta}\right)\left(\dfrac{2}{3}-\bd_n:\bd_n\right)}>\abs{\bar{\beeta}:\bd_n}$.
The other root is discarded since it may be negative.

To minimise $z^\text{trial}$, which is a univariate function of $r$ in the trial state, one can derive
\begin{gather}
...
\end{gather}
which, in the general case, can be solved by the Newton--Raphson method.
...
\end{multline}
Its derivative with regard to $r$ is
...
Compared to recent work \cite{Hashiguchi2018,Anjiki2019}, we present here an equivalent but simpler approach to compute the starting normal yield ratio for arbitrary loading increments.
...
Noting that $\dot{\balpha}$ is a linear function of $\balpha$, one can take advantage of the implicit Euler method to express $\balpha$ in explicit form.
...
\end{gather}
On the other hand, the deviatoric stress $\bs$ is computed from the trial stress $\bsigma^\text{trial}$,
\begin{gather}
...
Although it is not possible to simplify further, one should note that $\bzeta$ is a function of $\bvarepsilon$, $\gamma$, and $z$ only.
...
Taking the local variable as $\mb{x}=\begin{bmatrix}
        \gamma & z
    \end{bmatrix}^\mT$, the Jacobian $\mb{J}$ can be written as
\begin{gather}
    \mb{J}=\begin{bmatrix}
...
\begin{gather}
    ...
    \end{bmatrix}.
\end{gather}
By solving \eqsref{eq:local_equilibrium}, one can obtain $\ddfrac{\mb{x}}{\bvarepsilon}$. We are interested in the first row, which can be written explicitly as
\begin{gather}
...
The following is a reference implementation in C++.
Note that the alternative formulation presented in \tabref{tab:summary_alternative} is implemented.
\begin{cppcode}
...
Compared to previous implementations \cite{Fincato2017,Anjiki2019}, the formulation proposed in this work dramatically reduces the size of the local system from \num{20} to \num{2}.
...
Accounting for this factor scattered here and there would be cumbersome and error-prone, and thus requires excessive attention.
...
Now that we have introduced the split (e.g., \eqsref{eq:decomposition_back}) that accounts for magnitude and direction separately, we can consider additional tweaks.
...
Now consider both \eqsref{eq:decomposition_back} and \eqsref{eq:normalised_back_rule}. What we have at hand is essentially a tensor $\bbeta$ expressed as the product of a scalar $H$ and a tensor $\balpha$.
However, this tensor $\balpha$ is not a unit tensor; its saturated value is $\sqrt{\dfrac{2}{3}}$ in the 3D formulation.
Since we already have the scalar $H$, applying the above idea, we can formulate the evolution rule for $\bbeta$ as follows for the 3D case,
\begin{gather}
...
It is believed that this alternative formulation better unifies the uniaxial and multiaxial cases with minimal changes.
\begin{table}[htb]
    ...
    \end{tabular}
\end{table}
This alternative formulation makes it trivial to convert an existing implementation from the uniaxial case to the multiaxial case, or vice versa.
One only needs to scale the scalar bounds and $\dot{q}$ by $\sqrt{\dfrac{2}{3}}$. This factor can be abstracted into a constant variable that takes the value \num{1} for uniaxial cases and $\sqrt{\dfrac{2}{3}}$ for multiaxial cases.
The remaining parts of the implementation can largely remain unchanged.
