...
The original proposal of the subloading surface model introduces a new internal variable, called the normal yield ratio, denoted by $z$ in this work, which effectively scales the yield surface $f$ down by a factor $z$, the new surface is called the subloading surface $f_s$ and it is concentric with the yield surface $f$.
\begin{gather}
    f_s=\norm{\beeta}-zF.
\end{gather}
...
Since we already have the scalar $H$, applying the above idea, we can formulate the evolution rule for $\bbeta$ as follows for 3D case,
\begin{gather}
...
\begin{table}[htb]
    ...
    \end{tabular}
\end{table}
This alternative formulation makes it trivial to convert an existing implementation for uniaxial/multiaxial to the other case.
One only need to scale the scalar bounds and $\dot{q}$ by $\sqrt{\dfrac{2}{3}}$, this factor can be abstracted into a constant variable that takes value \num{1} for uniaxial and $\sqrt{\dfrac{2}{3}}$ for multiaxial cases.
