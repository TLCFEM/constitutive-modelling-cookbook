...
\begin{gather}
\dot{d}=\dot{c}-\dot{\alpha}=c_e\gamma\left(z_e\sigma^yn-\left(c-\alpha\right)\right)+\varPhi{}\left(c-\alpha\right)=c_e\gamma\left(z_e\sigma^yn-d\right)+\varPhi{}d.
\end{gather}
In the above, we have introduced two constants $c_e$ and $z_e$ to control the behaviour.
In recent formulations \cite{Hashiguchi2023}, the additional term $\varPhi{}d$ introduced with $\varPhi=\varPhi\left(\gamma\right)$ being a function of $\gamma$ solely.
We ignore it by setting $\varPhi=0$ in our model.
\subsection{Theory}
