...
Instead of \eqsref{eq:af_kinematic}, the back stress shall be defined as
\begin{gather}
\alpha=\sum_{i=1}^n\alpha_i+\sigma^t\left(1-e^{-m^tr}\right)n.
\end{gather}
This expression includes an additional `transition' term $\sigma^t\left(1-e^{-m^tr}\right)$, similar to the reduction term introduced previously in isotropic hardening.
If one forces $\sigma^t=\sigma^r$, it effectively means the reduction part from isotropic hardening is converted to kinematic hardening.
The total stress would then not be decreased.
If one sets $\sigma^t<\sigma^r$, then this transition is partial, the overall stress would still face a reduction as intended.
