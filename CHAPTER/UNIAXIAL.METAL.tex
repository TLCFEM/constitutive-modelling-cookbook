...
Here, for illustration, we adopt the linear evolutions of both isotropic and kinematic hardening as shown in \eqsref{eq:linear_combined}.
Then the subloading surface can be expressed as
\begin{gather}
f_s=\abs{\eta_{n+1}^\text{trial}}-\left(E+H\right)\gamma-z\left(\sigma^i+Kq_n+K\gamma\right).
\end{gather}
Enforcing it to be zero, along with the evolution of $z$, the local system consists of two scalar equations.
\begin{gather}
\mb{R}=\left\{
\begin{array}{l}
\abs{\eta_{n+1}^\text{trial}}-\left(E+H\right)\gamma-z\left(\sigma^i+Kq_n+K\gamma\right),\\
...
\gamma&z
\end{bmatrix}^\mT$, the Jacobian shall be written as
\begin{gather}
\mb{J}=\begin{bmatrix}
-E-H-zK&-\sigma^y\\
-U&1-\gamma{}U'
