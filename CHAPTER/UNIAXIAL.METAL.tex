...
\begin{gather}
\sigma^y=\sigma^i+Kq+\sigma^s\left(1-e^{-mq}\right),
\end{gather}
...
Also,
\begin{gather}
\dot{\sigma^y}=m\sigma^se^{-mq}=m\left(\sigma^i+\sigma^s-\sigma^y\right).
\end{gather}
...
\begin{gather}
\sigma^y=\sigma^i+Kq+\sigma^s\dfrac{\sum_{j=1}^{n}s_j\left(1-e^{-m_jq}\right)}{\sum_{j=1}^{n}s_j}.
\end{gather}
The fraction $\dfrac{\sum_{j=1}^{n}s_j\left(1-e^{-m_jq}\right)}{\sum_{j=1}^{n}s_j}$ defines a nonlinear curve that originates at point $\left(0,1\right)$ and asymptotically approaches zero when $q$ approaches infinity.
The parameter pairs $m_j$ and $s_j$ can be tweaked to approximate curves of \textbf{any} shape, theoretically.
...
\toprule
Constitutive Law&$\sigma=E\left(\varepsilon-\varepsilon^p\right)$\\
Yield Function&$f=\abs{\sigma-\alpha}-\left(\sigma^i+Kq+\sigma^s\left(1-e^{-mq}\right)\right)$\\
Flow Rule&$\dot{\varepsilon^p}=\gamma~\sign{\sigma-\alpha}$\\
Hardening Law&$\dot{q}=\gamma$\\
&$\alpha=\sum_{i=1}^n\alpha_i$\\
...
The trial stress can be computed following \eqsref{eq:uniaxial_trial_stress}. Then by denoting $\eta^\text{trial}=\sigma^\text{trial}-\alpha_n$, the yield function becomes
\begin{gather}
f^\text{trial}=\abs{\eta^\text{trial}}-\left(\sigma^i+Kq_n+\sigma^s\left(1-e^{-mq_n}\right)\right).
\end{gather}
\subsubsection{Plastic Evolution}
...
\begin{split}
f\left(\gamma\right)&=\abs{\eta_{n+1}}-\left(\sigma^i+Kq_{n+1}+\sigma^s\left(1-e^{-mq_{n+1}}\right)\right)\\
...
\end{gather}
where $w$ is a model constant that controls the rate of reduction.
...
A further surface shall be adopted.
\begin{gather}
	h=\abs{\varepsilon^p-\theta}-r,
\end{gather}
where $\theta$ is similar to $\alpha$, which, in this case, points to the centre of the non-hardening strain region. It may be called back plastic strain.

If one chooses linear evolution, denoting $n_h=\sign{\varepsilon^p-\theta}$, then
\begin{gather}
\dot{r}=K_r\nu,\qquad
\dot{\theta}=K_\theta\nu\pdfrac{h}{\varepsilon^p}=\nu{}n_hK_\theta,
\end{gather}
in which $K_r$ and $K_\theta$ are hardening moduli of $r$ and $\theta$ respectively, $\nu$ is the plastic multiplier, similar to $\gamma$.
\subsection{Formulation}
...
\begin{gather}
	n_h\left(\abs{\varepsilon^p_{n+1}-\theta_{n+1}}+\nu{}K_\theta\right)=\varepsilon^p_{n+1}-\theta_n.
\end{gather}
Noting that $\nu\geqslant0$ and $K_\theta\geqslant0$, one can conclude that
\begin{gather}
	\abs{\varepsilon^p_{n+1}-\theta_{n+1}}=\abs{\varepsilon^p_{n+1}-\theta_n}-\nu{}K_\theta,
\end{gather}
and
\begin{gather}
...
so that
\begin{gather}
	\nu=\dfrac{\abs{\varepsilon^p_{n+1}-\theta_n}-r_n}{K_\theta+K_r}.
\end{gather}

It is easy to see that, for the excessive plastic strain that exceeds the current non-hardening strain region, this amount of plastic strain will be distributed to $\theta$ and $r$ as
\begin{gather}
	\dot{r}=\dfrac{K_r}{K_\theta+K_r}\left(\abs{\varepsilon^p_{n+1}-\theta_n}-r_n\right),\\
	\dot{\theta}=n_h\dfrac{K_\theta}{K_\theta+K_r}\left(\abs{\varepsilon^p_{n+1}-\theta_n}-r_n\right).
\end{gather}

Accounting for the Kuhn--Tucker condition, one may further unify the cases to obtain
\begin{gather}
\dot{r}=\dfrac{K_r}{K_\theta+K_r}\left\langle\abs{\varepsilon^p_{n+1}-\theta_n}-r_n\right\rangle,\\
\dot{\theta}=n_h\dfrac{K_\theta}{K_\theta+K_r}\left\langle\abs{\varepsilon^p_{n+1}-\theta_n}-r_n\right\rangle.
\end{gather}
\subsection{Implementation}
