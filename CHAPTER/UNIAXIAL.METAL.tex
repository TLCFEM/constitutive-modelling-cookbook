...
A multiplicative formulation \cite{Chaboche1989} for back stress is adopted with the Armstrong--Frederick type hardening rule. The back stress is defined to be the summation of several back stresses that evolve independently with different rates. That is,
\begin{gather}
...
Theoretically, other higher order (multi-step) numerical methods for ODEs can also be applied.
...
The discussed Armstrong--Frederick model allows a versatile modelling of kinematic hardening.
...
\end{array}\right.
\end{gather}
The term $K\varepsilon^p$ defines the linear hardening, which serves as a baseline, given that the yield stress $\sigma_y^+$ ($\sigma_y^-)$ defines the boundary, the fraction is always smaller than unity.
\subsubsection{Hardening Law}
...
\end{cppcode}
\section{VAFCRP1D}
Finally, an extension of Armstrong--Frederick model is presented to close this chapter. A 3D version will be introduced later.
\subsection{Theory}
\subsubsection{Yield Function}
