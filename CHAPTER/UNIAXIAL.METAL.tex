...
The extended version further introduces the concept of elastic core, which is characterised by an additional variable $c$ such that the shifted stress $\eta$ shall be computed as follows instead.
\begin{gather}
...
The evolution of $d$ takes a complex form that shall be expressed as
\begin{gather}
\begin{split}
\dot{d}=\dot{c}-\dot{\alpha}&=c_e\dot{q}\left(\dfrac{z_e}{z}\eta-\left(c-\alpha\right)\right)+\dfrac{\dot{\sigma^y}}{\sigma^y}\left(c-\alpha\right)\\
&=c_e\gamma\left(\dfrac{z_e}{z}\eta-d\right)+\dfrac{\dot{\sigma^y}}{\sigma^y}d.
\end{split}
\end{gather}
Since numerical world does not like division, the above is equivalently expressed as
\begin{gather}
z\dot{d}=c_e\gamma\left(z_e\eta-zd\right)+\dfrac{\dot{\sigma^y}}{\sigma^y}zd.
\end{gather}
In the above, we have introduced two constants $c_e$ and $z_e$ to control the behaviour.
\subsection{Theory}
The residual for $d$ shall be expressed as
\begin{gather}
\begin{split}
&z\left(d_n+\dot{d}-d\right)\\
=&zd_n-zd+c_e\gamma\left(z_e\eta-zd\right)+\dfrac{\dot{\sigma^y}}{\sigma^y}zd,\\
=&zd_n+c_ez_e\gamma{}\eta+\left(\dfrac{\dot{\sigma^y}}{\sigma^y}-c_e\gamma-1\right)zd.
\end{split}
\end{gather}

Along with the subloading surface $f_s$ and $z$, treating $\eta$ as an independent variable (purely for numerical convenience), one could formulate the local system with four residuals.
\begin{gather}
\mb{R}=\left\{
\begin{array}{l}
\sigma-\alpha+\left(z-1\right)d-\eta\\[4mm]
\abs{\eta}-z\sigma^y,\\[4mm]
z-z_n+u\ln\left(z\right)\gamma,\\[4mm]
zd_n+c_ez_e\gamma{}\eta+\left(\dfrac{\dot{\sigma^y}}{\sigma^y}-c_e\gamma-1\right)zd.
\end{array}
\right.
\end{gather}
...
\gamma&\eta&z&d
\end{bmatrix}^\mT$, denoting $n=\sign{\eta}$, the Jacobian shall be written as
\begin{gather}
\mb{J}=\begin{bmatrix}
-En-\ddfrac{\alpha}{\gamma}&-1&d&z-1\\[4mm]
-z\ddfrac{\sigma^y}{\gamma}&n&-\sigma^y&\cdot\\[4mm]
u\ln\left(z\right)&\cdot&1+\dfrac{u\gamma}{z}&\cdot\\[4mm]
c_ez_e\eta+zd\ddfrac{\Omega}{\gamma}&c_ez_e\gamma&d_n+d\Omega{}&z\Omega{}
\end{bmatrix}.
\end{gather}
In the above,
\begin{gather}
\Omega=\dfrac{\dot{\sigma^y}}{\sigma^y}-c_e\gamma-1.
\end{gather}
...
Thus,
\begin{gather}
    \dot{\sigma^y}=\left(k_\text{iso}+\sigma^s_\text{iso}m^s_\text{iso}e^{-m^s_\text{iso}q}\right)\gamma.
\end{gather}

Foe kinematic hardening, we use the Armstrong--Frederick rule.
\begin{gather}
    \dot{\alpha}=m^s_\text{kin}\gamma\left(a^yn-\alpha\right).
\end{gather}
...
\begin{gather}
    \alpha=\dfrac{m^s_\text{kin}\gamma{}a^yn+\alpha_n}{1+m^s_\text{kin}\gamma}.
\end{gather}

The derivative can be derived as
\begin{gather}
    \ddfrac{\alpha}{\gamma}=m^s_\text{kin}\dfrac{na^y-\alpha_n+n\gamma{}\ddfrac{a^y}{\gamma}\left(1+m^s_\text{kin}\gamma\right)}{\left(1+m^s_\text{kin}\gamma\right)^2}.
\end{gather}
