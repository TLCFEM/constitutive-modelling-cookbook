...
Along with the subloading surface $f_s$ and $z$, treating $\eta$ as an independent variable (purely for numerical convenience), one could formulate the local system with three residual expressions.
\begin{gather}
\mb{R}=\left\{
\begin{array}{l}
\sigma-\alpha+\left(z-1\right)d-\eta\\
\abs{\eta}-z\sigma^y,\\
...
\begin{gather}
\mb{J}=\begin{bmatrix}
-En-\ddfrac{\alpha}{\gamma}&-1&d&z-1\\[3mm]
-\ddfrac{\sigma^y}{\gamma}&n&-\sigma^y&\cdot\\[3mm]
u\ln\left(z\right)&\cdot&1+\dfrac{u\gamma}{z}&\cdot\\[3mm]
c_ez_e\eta+zd\left(\Xi-c_e\right)&c_ez_e\gamma&d_n+\Omega{}d&\Omega{}z
\end{bmatrix}.
\end{gather}
In the above,
\begin{gather}
\Xi=\ddfrac{}{\gamma}\left(\dfrac{\dot{\sigma^y}}{\sigma^y}\right),\qquad
