...
Here we present a versatile viscoplastic model for metals under dynamic loading. It is essentially a multiaxial extension of the aforementioned Armstrong--Frederick uniaxial model with an additional viscosity component. Due to the presence of viscosity, the usual formulation and implementation should be modified slightly, because transient effects are now accounted for and the yield function does not need to remain non-positive. It is used only to determine whether a trial state is elastic or plastic.
\subsection{Theory}
...
\end{gather}
Strictly speaking, this should be the viscoplastic strain rather than the plastic strain. For brevity, however, the superscript $\left(\cdot\right)^p$ is used. If desired, $\left(\cdot\right)^{vp}$ can also be used.
...
\end{gather}
in which $\Delta{}t$ is the increment of pseudo-time, which should be available from the global time integration method, and $\mu$ and $\epsilon$ are two material constants. Here a \cite{Peric1993} type rule is used for easier numerical implementation. Equivalently, after some rearrangement, it becomes
\begin{gather}
...
\end{gather}
In which, $m$ is a parameter that controls the speed of hardening.
...
\end{gather}
Now both $\beeta$ and $\bn$ are functions of $\gamma$, which simplifies the local system. Based on this fact,
\begin{gather}
...
The above viscoplasticity models often require an explicit reference to time $t$ (and its increment $\Delta{}t$). Given the strain increment $\Delta{}\bvarepsilon$ and $\Delta{}t$, the strain rate can be computed by, for example, assuming a constant rate. In this way, the whole model can be converted into a form that depends only on strain input, provided that (pseudo-)time is always accessible to local material points. Even with a static analysis setup, creep can be modelled.

Here we present a general framework \cite{Chang2022a} to solve the viscoplasticity model that is truly responsive to both strain and strain-rate inputs.
...
\end{gather}
It should be noted that \eqsref{eq:quadrant_damper_govern} can also be expressed as
\begin{gather}
...
The definition of $\eta\left(\varepsilon,\dot\varepsilon\right)$ can be quite flexible to describe the desired response. Such a modification mimics semi-active control schemes in which stress feedback can be adjusted based on different strain and strain rates as inputs. For more complex (semi-)active schemes, it can be further defined as a function of other quantities, such as system energy and its history. Two examples are shown as follows to illustrate this feature.
\subsubsection{Proposed Quadrant Modification}\label{sec:quadrant_damper}
The simplest case uses four different constants for the four quadrants of the $\varepsilon$-$\dot{\varepsilon}$ space.
...
Since a sudden change in damping response is not achievable in practice, the constants $g_1$ and $g_2$ can represent the physical manufacturing tolerances of pistons and chambers, strictly speaking mainly the one associated with displacement tolerance, viz., $g_1$.
...
\end{gather}
It should be noted that the derivatives of \eqsref{eq:arctan_transition} have a simpler form than those of the above definition. Other simple functions such as a linear function can also be applied, in which the transition range can be explicitly defined.
...
\end{gather}
The subscripts $\cdot_d$ and $\cdot_s$ represent the dashpot and spring parts, respectively. By differentiating the total strain with respect to time, one obtains
...
\end{enumerate}
More accurate results can be achieved with fewer function evaluations and thus less computation time if a better solution method is available. The ideal algorithm should strictly comply with the theory and achieve a higher convergence rate. Moreover, there is still room for improvements of both applicability and efficiency/robustness.
...
\end{gather}
where $\beta$ is an integration parameter. For $\beta=0.5$, a constant acceleration rule is implied. Since there is no other constraint imposed, such an integration relationship can be alternatively applied to either spring component ($\chi=\varepsilon_s$) or dashpot component ($\chi=\varepsilon_d$). In this way, kinematic compatibility can be satisfied rigorously within the Maxwell model and remains independent of the global time integration method. The parameter $\beta$ can be defined as a user input (if $\chi=\varepsilon_s$ or $\chi=\varepsilon_d$) or be solved internally (if $\chi=\varepsilon$), which is
...
With the above basic formulae at hand, the problem can now be rephrased as follows: knowing $\varepsilon^n$, $\dot\varepsilon^n$, $\varepsilon^n_d$, $\dot\varepsilon^n_d$, $\varepsilon^n_s$, and $\dot\varepsilon^n_s$, and given the total increments $\Delta\varepsilon=\varepsilon^{n+1}-\varepsilon^n$ and $\Delta\dot\varepsilon=\dot\varepsilon^{n+1}-\dot\varepsilon^n$, find the increments $\Delta\varepsilon_d$, $\Delta\varepsilon_s$, $\Delta\dot\varepsilon_d$, and $\Delta\dot\varepsilon_s$ that satisfy
\begin{gather}
...
\end{bmatrix}.
\end{gather}
It should be pointed out that, for brevity, the parameter $\beta$ is assumed to be a constant user input in the above derivation. If it is expressed as a function of $\mb{a}$ via \eqsref{eq:delta}, the corresponding $\partial\mb{x}/\partial\mb{a}$ can be derived accordingly. The procedure is presented in the appendix. The stiffness and damping moduli can be readily computed by using the chain rule. Knowing that
...
Numerical difficulties may arise if $\partial\eta/\partial\varepsilon_d$ becomes too large because the steepness parameter $g_1$ is chosen too large. The same situation occurs with $\partial\eta/\partial\dot\varepsilon_d$ and $g_2$. This problem can be eased by scaling the steepness parameters according to the maximum strain and strain rate. Another major problem is that $\partial\sigma_d/\partial\dot\varepsilon_d$ approaches infinity at the origin if the exponent $\alpha$ in the power-law model is smaller than unity. A strain rate sufficiently close to the origin is often encountered, especially in forced vibrations. This may not be a problem if the corresponding damping modulus term $\partial\sigma_d/\partial\dot\varepsilon_d$ is not involved in the global equation of motion, in which case the quadratic convergence rate cannot be recovered. However, when combined with a spring to form a Maxwell model, a disproportionately large $\partial\sigma_d/\partial\dot\varepsilon_d$ may cause the local iteration to fail. A possible workaround would be limiting the apparent viscosity to a finite value via extrapolation \cite{Wu1998}.
