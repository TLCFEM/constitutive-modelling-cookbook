...
The effective stress $\bbsigma$ obeys the conventional hardening model using the Hoffman yielding criterion, see \secref{sec:hoffman} for details. The final stress can be obtained after applying damage factors to the tensile and compressive parts of $\bbsigma$.
\begin{gather}
...
\end{gather}
that covers both tensile and compressive cases. The matrix $\mb{H}_\aleph$ is the projection matrix of the Hill criterion, which is a special case of \eqsref{eq:hoffman_criterion}. For $\aleph=t$, set $f_{ii}^c=f_{ii}^t$. For $\aleph=c$, set $f_{ii}^t=f_{ii}^c$. It should be noted that in either case, $\mb{q}=\mb{0}$, thus, \eqsref{eq:timber_damage} does not contain a second term.
