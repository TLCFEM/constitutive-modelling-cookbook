...
\bar{J_3}=I_3^{1/2}.
\end{gather}
Here, $I_1$, $I_2$, and $I_3$ are the invariants of $\mb{C}$.
\section{Mooney--Rivlin Model}
\subsection{Theory}
The Mooney--Rivlin model for a compressible material uses the following strain energy density function
\begin{gather}
...
As one may observe, since $\bar{J_3}$ is involved, the bulk modulus $K$ can be set to a large value so that the model approaches so-called near-incompressible behaviour.
...
\end{split}
\end{gather}
One should realise that the above result agrees with the expression derived using tensor algebra.
...
One should compute the derivatives faithfully and then assemble the stress and stiffness tensors.
\begin{cppcode}
...
\section{Yeoh Model}
\subsection{Theory}
The Yeoh model for compressible materials is given by
\begin{gather}
