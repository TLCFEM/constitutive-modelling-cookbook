\chapter{Other}
\section{Gurson's Void Growth Model}
We present an extended version \cite{Tvergaard1984} of Gurson's void growth model \cite{Gurson1977}.
It is slightly different from the one discussed by \cite{SouzaNeto2008} in Chapter 12.5.
It is (almost) identical to the one implemented in ABAQUS (see, for example, \S~4.3.6 in \cite{ABAQUS2011}).

As suggested \cite{ABAQUS2011}, it is `\textit{intended for use with mildly voided metals}'.
In principle, it is suitable for porous materials in which damage is mainly driven by the development of voids.
However, since a von Mises-like yield criterion is used, modelling materials that do not follow the von Mises rule may not be appropriate.
\subsection{Theory}
...
This element fuses the concepts of element, section, and material, and can therefore also be regarded as a material model.

However, for the same reason, calibration can only be carried out at the beam-element level. In this regard, one may argue that conventional fibre/material-based elements are inherently superior, and this is often true. However, fibre-based elements cannot easily capture section-level effects, which largely depend on the formulation of the specific beam element used. On the other hand, the element introduced in this section allows flexible customisation of response, which may not necessarily be strictly tied to the underlying material response. This is beneficial for engineering design, because design standards often adopt much more conservative $N$-$M$ envelopes, which can differ substantially from the `actual' envelope derived from material models.
...
The main purpose of adopting $\be$ in the formulation is twofold: 1) to decouple nodal response so that plasticity (on rotational DoFs) can be developed \textbf{independently} at each end, and 2) to obtain a better implication of the corresponding plastic deformation.
...
However, the two--surface concept (yield surface and bounding surface) is not adopted, because its presence makes the quantification of hardening behaviour difficult.
...
In the following, we present a special formulation that allows independent activation of plasticity at each end while enabling proper hardening behaviour.
\subsubsection{Yield Function}
...
The $N$-$M$ interaction surface can not only change its location, governed by $\bbeta_\aleph$, but also grow in size, governed by $\alpha_\aleph$.
...
\end{figure}
\subsubsection{Flow Rule}
The evolution of plastic deformation $\overline{\be}^p$ should be linked to the gradient $\mb{g}$ of the plastic potential, which is simply taken as $f$, leading to $\mb{g}=\pdfrac{f}{\mb{\overline{q}}}$.
...
For isotropic hardening, $\dot{\overline{\alpha}_\aleph}$ should be related to some scalar measure of the plastic deformation $\dot{\overline{\be}^p}$.
...
\end{figure}
\paragraph{Armstrong--Frederick Type}
By choosing $D=K_a\norm{\mb{\zeta}}$, the above bounding type hardening rule falls back to the Armstrong--Frederick type \cite{Frederick2007} kinematic hardening that can be expressed as
...
\item Conventional fibre-based beam elements have symmetric tangent stiffness matrices.
...
The discrete local system in vector form is derived in this section for 3D beam elements, accounting for five elemental deformation/force components, which are equivalent to three nodal components.
...
The trial yield function can then be evaluated for each node.
...
If $f^\text{trial}=\sum{}\langle{}f_\aleph^\text{trial}\rangle=0$, then both $\Phi_\aleph^\text{trial}$ are non-positive, indicating that both ends undergo elastic loading/unloading.
Otherwise, $f^\text{trial}>0$, and local return mapping is required to determine the plastic state so that the conditions $\gamma>0$ and $f=0$ are satisfied.
...
Thus, each end should be considered separately, though not strictly independently because of the shared axial force.
