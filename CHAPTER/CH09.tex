...
\begin{tabular}{r|l|l}
    \toprule
                        & previous model \cite{Kostic2013}                                  & revised model                                                                                                                                                   \\ \midrule
...
\begin{enumerate}
\item The previous model is developed on dimensional quantities $\bv$ and $\mb{q}$, but the yield surface is non-dimensional, resulting in the associative plastic flow be normalised by yield deformation (dimensionless).
The hardening response then becomes dependent on element rigidities and yield forces.
The new model is formulated in a non-dimensional space ($\overline{\be}$ and $\mb{\overline{q}}$), which corrects hardening response.
\item The previous model uses \textbf{two} yield functions, each corresponds to an interaction surface.
...
\item The previous model uses two plastic multipliers ($\gamma_\aleph$) due to two yield surfaces, which causes local bifurcation issue when two nodes are experiencing identical axial plastic history.
The proposed model uses one $\gamma$ and is free from local numerical instability.
\item The previous model uses a constant $H$ for isotropic hardening, but fails to generate linear isotropic hardening behaviour due to non-homogeneous attribute of interaction surface.
The proposed model revises the definition of isotropic hardening and manages to generate desired isotropic hardening response.
...
\item The previous model further adopts a limiting/bounding surface, which originates from the generalised plasticity framework in the original literature \cite{Auricchio1994}, to generate nonlinear hardening response.
The new model achieves the same response by adopting an Armstrong--Fredrick type kinematic hardening rule instead.
