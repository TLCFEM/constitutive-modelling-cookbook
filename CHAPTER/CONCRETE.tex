...
The effective stress $\bbsigma_{n+1}$ only depends on $\bvarepsilon_{n+1}$ and $\gamma$.
\begin{gather}
\begin{split}
...
For compression, the definition fully resembles its tensile counterpart. The equivalent strain is driven by two internal parameters $\kappa_{dc1}$ and $\kappa_{dc2}$,
\begin{gather}
...
\begin{gather}
R=f_c\exp\left(-\dfrac{\varepsilon_i}{\varepsilon_{fc}}\right)-\left(1-\omega_c\right)E\kappa_{dc}.
\end{gather}
