...
In this section, we present the concrete damage plasticity model proposed by \cite{Lee1998}.
It can be seen as a continuation and further extension of the Barcelona model \cite{Lubliner1989}, which proposes a unified uniaxial backbone curve with several interesting features.
ABAQUS implements a slightly different version, with Lode angle dependency and other minor changes.
...
\begin{gather}
f=\alpha\bar{I}_1+\sqrt{\dfrac{3}{2}}\norm{\bar{\bs}}+\beta\left\langle\hat{\sigma}_1\right\rangle-\zeta\left\langle-\hat{\sigma}_1\right\rangle-\left(1-\alpha\right)c_c,
\end{gather}
...
The parameter $\zeta$ \cite{Lubliner1989} is determined by the ratio between the tensile and compressive meridian stresses $K_c$ such that
\begin{gather}
\zeta=\dfrac{3\left(1-K_c\right)}{2K_c-1}.
\end{gather}
A typical value $K_c=\dfrac{2}{3}$ yields $\zeta=3$.
One can further rewrite the yield function into the following concise form.
\begin{gather}
f=\alpha\bar{I}_1+\sqrt{\dfrac{3}{2}}\norm{\bar{\bs}}+\psi\hat{\sigma}_1-\left(1-\alpha\right)c_c,
\end{gather}
with
\begin{gather}
\psi=\left\{
...
It should be noted that, for a given $\bbsigma$, one can perform the eigendecomposition to obtain the principal stress $\hat{\bsigma}$, for which the following holds
\begin{gather}
\hat{I}_1=\tr{\hat{\bsigma}}=\bar{I}_1,\qquad
\norm{\hat{\bs}}=\norm{\bar{\bs}},
\end{gather}
because these quantities are invariants of the given tensor.
In the following, we do not distinguish between these two forms of invariants.
The same quantity can be computed using different tensors and, depending on the context, one form may be more convenient than the other.
...
This is a Drucker--Prager function as later seen in \secref{sec:drucker_prager}.
Some implementations add an extra term to the square root to get rid of the apex such that the surface is well-defined.
It is not considered here for two reasons: a) the apex return is practically not reachable, and b) the extra term breaks coaxiality thus further complicates the implementation.
...
One can check \secref{sec:derivative_eigen} for detailed derivations and explanations.
...
The internal hardening parameters $\kappa_\aleph$ are governed by the following evolution rule,
\begin{gather}
\dot{\kappa}_\aleph=\gamma{}H_\aleph,
\end{gather}
where $H_\aleph$ defines the hardening law. Different $H_\aleph$ are used for compression and tension.
\begin{gather}
...
It must be noted that since $\hat{\bn}$ is the normalised principal deviatoric stress tensor, it is guaranteed that $\hat{n}_1>0>\hat{n}_3$.
However, due to the presence of $\alpha_p$, $\hat{n}_3+\alpha_p$ may not be strictly negative.
On the other hand, as a plasticity history, $\kappa_c$ should be non-decreasing, implying $\dot{\kappa}_c\geqslant0$, forcing $\hat{n}_3+\alpha_p\leqslant0$.
A more rigorous definition shall then be
\begin{gather}
    \dot{\kappa}_\aleph=\gamma{}\left\langle{}H_\aleph\right\rangle.
\end{gather}
This is only relevant to the compression case, the tension case is always valid.
Here we simply use a unified expression for brevity.
...
\begin{gather}\label{eq:cdp_d_backbone}
...
However, this issue warrants explicit consideration.
In \eqsref{eq:cdp_d_backbone}, when $c_\aleph/b_\aleph<1$, which may occur for certain combinations of input parameters, the derivative (slope) of $d_\aleph$ diverges as $\kappa_\aleph\rightarrow1$.
This can lead to severe local convergence difficulties that are neither physically meaningful nor relevant, but instead introduce unnecessary numerical complications, particularly because the corresponding strength becomes negligible as $\kappa_\aleph\rightarrow1$.
A simple remedy is to impose an upper bound on the slope, for example,
\begin{gather}
    \ddfrac{d_\aleph}{\kappa_\aleph}\leqslant100.
\end{gather}
Whenever this constraint is violated, the original curve is replaced by a linear segment with a slope of \num{100}.

In general, the backbone curve can be customised. The main algorithm is not concerned with how the backbone curve is computed; it only requires $f_\aleph$, $\bar{f}_\aleph$, and $d_\aleph$, together with their derivatives with respect to $\kappa_\aleph$. The exponential form adopted in the original model may encounter numerical difficulties, which, in the author's opinion, is not ideal. Other forms can always be chosen, such as providing these quantities in tabulated form. This is also what ABAQUS offers.
...
Hardening Law&$\dot{\kappa}_\aleph=\gamma{}\left\langle{}H_\aleph\right\rangle$\\
\bottomrule
\end{tabular}
\end{table}
...
f^\text{trial}=\alpha\bar{I}_1^\text{trial}+\sqrt{\dfrac{3}{2}}\norm{\bar{\bs}^\text{trial}}+\psi\hat{\sigma}_1^\text{trial}-\left(1-\alpha\right)c_c,
\end{gather}
...
\mb{R}=\left\{
\begin{array}{l}
\alpha\bar{I}_1+\sqrt{\dfrac{3}{2}}\norm{\bar{\bs}^\text{trial}}-\gamma\sqrt{6}G+\psi\left(\hat{\sigma}_1^\text{trial}-\gamma\left(2G\hat{n}_1+3K\alpha_p\right)\right)+\left(1-\alpha\right)\bar{f}_c,\\[4mm]
\kappa_{t,n}+\gamma{}r\left\langle\dfrac{f_t}{g_t}\left(\hat{n}_1+\alpha_p\right)\right\rangle-\kappa_t,\\[4mm]
\kappa_{c,n}+\gamma\left(1-r\right)\left\langle\dfrac{f_c}{g_c}\left(\hat{n}_3+\alpha_p\right)\right\rangle-\kappa_c.
\end{array}
\right.
\end{gather}
...
\mb{J}=\begin{bmatrix}
-9K\alpha\alpha_p-\sqrt{6}G-\psi\left(2G\hat{n}_1+3K\alpha_p\right)&\hat{\sigma}_1\pdfrac{\psi}{\kappa_t}&\left(1-\alpha\right)\bar{f}_c'+\hat{\sigma}_1\pdfrac{\psi}{\kappa_c}\\[4mm]
f_t\left\langle\dfrac{\hat{n}_1+\alpha_p}{g_t}\right\rangle\left(r+\gamma\pdfrac{r}{\gamma}\right)&\gamma{}r\left\langle\dfrac{\hat{n}_1+\alpha_p}{g_t}\right\rangle{}f_t'-1&\cdot\\[4mm]
f_c\left\langle\dfrac{\hat{n}_3+\alpha_p}{g_c}\right\rangle_-\left(1-r-\gamma\pdfrac{r}{\gamma}\right)&\cdot&\gamma\left(1-r\right)\left\langle\dfrac{\hat{n}_3+\alpha_p}{g_c}\right\rangle_-f_c'-1
\end{bmatrix}
\end{gather}
\end{footnotesize}
with
\begin{gather*}
\pdfrac{\beta}{\kappa_t}=\left(1-\alpha\right)\dfrac{\bar{f}_c}{\bar{f}_t^2}\bar{f}_t',\qquad
...
f_t\left\langle\dfrac{\hat{n}_1+\alpha_p}{g_t}\right\rangle\left(r+\gamma\pdfrac{r}{\gamma}\right)&r\gamma\left\langle\dfrac{\hat{n}_1+\alpha_p}{g_t}\right\rangle{}f_t'-1&\cdot\\[4mm]
f_c\left\langle\dfrac{\hat{n}_3+\alpha_p}{g_c}\right\rangle_-\left(1-r-\gamma\pdfrac{r}{\gamma}\right)&\cdot&\left(1-r\right)\gamma\left\langle\dfrac{\hat{n}_3+\alpha_p}{g_c}\right\rangle_-f_c'-1
\end{bmatrix},
\end{gather}
\end{footnotesize}
otherwise,
...
\begin{gather}
\mb{J}=\begin{bmatrix}
-9K\alpha\alpha_p-\sqrt{6}G-\zeta\left(2G\hat{n}_1+3K\alpha_p\right)&\cdot&\left(1-\alpha\right)\bar{f}_c'\\[4mm]
f_t\left\langle\dfrac{\hat{n}_1+\alpha_p}{g_t}\right\rangle\left(r+\gamma\pdfrac{r}{\gamma}\right)&r\gamma\left\langle\dfrac{\hat{n}_1+\alpha_p}{g_t}\right\rangle{}f_t'-1&\cdot\\[4mm]
f_c\left\langle\dfrac{\hat{n}_3+\alpha_p}{g_c}\right\rangle_-\left(1-r-\gamma\pdfrac{r}{\gamma}\right)&\cdot&\left(1-r\right)\gamma\left\langle\dfrac{\hat{n}_3+\alpha_p}{g_c}\right\rangle_-f_c'-1
\end{bmatrix}.
\end{gather}
\end{footnotesize}
In the above, $\left\langle\cdot\right\rangle_-$ denotes the negative--part operator, corresponding to the Macaulay brackets $\left\langle\cdot\right\rangle$.
...
\begin{array}{l}
3K\alpha{}\mb{1}+\sqrt{6}G\bn+\psi\ddfrac{\hat\sigma_1}{\bbsigma}:\pdfrac{\bbsigma}{\bvarepsilon_{n+1}},\\[4mm]
...
H\left(H_c\right)\gamma\dfrac{f_c}{g_c}\left(\left(1-r\right)\pdfrac{\hat{n}_3}{\bvarepsilon_{n+1}}-\left(\hat{n}_3+\alpha_p\right)\ddfrac{r}{\bbsigma}:\pdfrac{\bbsigma}{\bvarepsilon_{n+1}}\right).
\end{array}
\right.
\end{gather}
Note the last row is only active when $H\left(H_c\right)=1$ with $H\left(\cdot\right)$ denotes the Heaviside step function, that is $H_c>0$.
Hence
\begin{gather}
...
The state determination of the CDP model is shown in \algoref{algo:cdp_model}. Compared with the original implementation, the one presented here is much more concise and avoids lengthy computation of the consistent tangent stiffness.

It is worth noting that some quantities remain constant during the local iteration, for example,
\begin{gather}
2G\hat{n}_1+3K\alpha_p,\qquad
\dfrac{\hat{n}_1+\alpha_p}{g_t},\qquad
\dfrac{\hat{n}_3+\alpha_p}{g_c}.
\end{gather}
They can be computed and stored before entering the local iteration.
...
The C++ implementation of the state determination is given below.
\begin{cppcode}
NonlinearCDP::update_trial_status
\end{cppcode}

A typical backbone computation can be seen as follows.
\begin{cppcode}
CDP::compute_backbone
\end{cppcode}
\section{Anisotropic CDP for Masonry}
Masonry and concrete bear similarities in terms of mechanics.
Concrete is often considered isotropic at the macro level.
Masonry is inherently anisotropic (orthotropic) due to horizontal mortar bed joints and vertical head joints creating preferential failure planes along mortar lines.
This implies that a constitutive model designed for concrete can be repurposed to model masonry with some minor modifications.
\subsection{Theory}
\subsubsection{Transformation}
Borrowing the idea of \secref{sec:ylp0418p}, the effective stress can be obtained via transformation.
\begin{gather}
    \bsigma^*=\mb{A}^+\bar\bsigma^++\mb{A}^-\bar\bsigma^-,
\end{gather}
where $\mb{A}^+$ and $\mb{A}^-$ are two constant transformation matrices applied to the tension and compression parts.
It can also be expressed as
\begin{gather}
    \bsigma^*=\mb{A}\bar\bsigma,
\end{gather}
where now $\mb{A}=\mb{A}\left(\bar\bsigma\right)$ is a function of $\bar\bsigma$.
Depending on the specific form of $\mb{A}$, a proper anisotropic (here orthotropic) yield surface can be constructed from the isotropic one.

A popular choice is to use the eigendecomposition of $\bar\bsigma$.
Let $\bar\bsigma=\underbrace{\mb{P}}_\text{\numproduct{6x3}}\hat{\bar\bsigma}$ where $\hat{\bsigma}$ is the principal stress, then
\begin{gather}
    \bar\bsigma^+=\mb{P}\left\langle\hat{\bsigma}\right\rangle,
\end{gather}
thus,
\begin{gather}
    \begin{split}
    \bsigma^*&=\mb{A}^+\mb{P}\left\langle\hat{\bsigma}\right\rangle+\mb{A}^-\mb{P}\left(\hat{\bsigma}-\left\langle\hat{\bsigma}\right\rangle\right)\\
    &=\left(\mb{A}^+-\mb{A}^-\right)\mb{P}\left\langle\hat{\bsigma}\right\rangle+\mb{A}^-\mb{P}\hat{\bsigma}.
    \end{split}
\end{gather}
\subsubsection{Yield Surface}
Hence, the same yield surface is used, but with the effective stress replaced by the transformed one.
\begin{gather}
f=\alpha{}I^*_1+\sqrt{\dfrac{3}{2}}\norm{\bs^*}+\psi\hat{\sigma}^*_1-\left(1-\alpha\right)c_c.
\end{gather}
In this case, the invariants are computed from the transformed stress $\bsigma^*$.
\begin{gather}
I^*_1=\tr{\bsigma^*},\qquad
\bs^*=\dev{\bsigma^*}.
\end{gather}
\subsubsection{Flow Rule}
Similarly, the same flow potential is chosen such that the flow rule can be expressed as
\begin{gather}
\dot{\bvarepsilon^p}=\gamma\pdfrac{g}{\bbsigma}=\gamma\left(\dfrac{\bs^*}{\norm{\bs^*}}+\alpha_p\mb{1}\right):\ddfrac{\bsigma^*}{\bar\bsigma}=\gamma\left(\bn+\alpha_p\mb{1}\right):\ddfrac{\bsigma^*}{\bar\bsigma}.
\end{gather}
Because of the transformation, the coaxiality no longer exists.
\section{CDPM2 Model}
The CDP model adopts isotropic damage, which leads, for example, to degradation of compressive/tensile strength due to tensile/compressive damage. In cyclic loading cases, this may not be ideal. The CDPM2 model uses a different approach that applies tensile damage to the tensile part of the stress and compressive damage to the compressive part.
...
\end{gather}
depends on the material strengths $f_c$ and $f_t$. Here, $e$ is the eccentricity constant, which depends on those two strengths and the equibiaxial compression strength $f_{bc}$. The hardening functions $q_{h1}$ and $q_{h2}$ are introduced later. The helper functions $g_1$ and $g_3$ are also used to define the plastic potential $G$. If the Lode angle is considered, $g_3$ should be replaced by $g_4$, which is defined as
\begin{gather}
...
\end{gather}
Assume an exponential degradation curve \cite{Xenos2016},
\begin{gather}
\sigma=f_t\exp\left(-\dfrac{\varepsilon_i}{\varepsilon_{ft}}\right)
\end{gather}

The local residual is
\begin{gather}\label{eq:cdpm2_dt_r}
...
\begin{gather}\label{eq:cdpm2_dc_r}
R=f_t\exp\left(-\dfrac{\varepsilon_i}{\varepsilon_{fc}}\right)-\left(1-\omega_c\right)E\kappa_{dc}.
\end{gather}
\subsection{Formulation}
The CDPM2 model employs a single internal variable, $\kappa_p$.
A key feature of the model is the complete decoupling of plasticity and damage, such that the evolution of either component does not affect the other.
...
\end{bmatrix}.
\end{gather}
\subsubsection{Damage Residual}
The damage residuals are shown in \eqsref{eq:cdpm2_dt_r} and \eqsref{eq:cdpm2_dc_r}.
A conventional Newton--Raphson iteration can be used to solve for $\omega_t$ and $\omega_c$.
