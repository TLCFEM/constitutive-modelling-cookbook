...
\section{K4 Concrete}
In this section, we introduce a uniaxial model for concrete formulated within the classic plasticity framework. In this book, we call it the K4 model because the four authors' surnames all start with the letter `K'. However, the model presented in the original paper adopts linear degradation, which inevitably leads to zero stiffness once a certain strain limit is exceeded. Numerical models do not favor exact zeros, so we revise it to use an exponentially decaying function instead. Note also that the original formulation \cite{Kenawy2020} contains some mistakes. In the event of any discrepancy or inconsistency, we follow the formulation discussed in this section.

There are quite a number of phenomenological models for concrete. Since they are phenomenological, different loading/unloading responses can be defined with relative flexibility. Such flexibility makes these models easier for engineers to understand and extrapolate. However, they may violate thermodynamic principles.
\subsection{Theory}
The classic plasticity framework does not support the definition of stiffness degradation; the elastic loading/unloading paths always follow the initial stiffness gradient. It is common to combine plasticity and damage \cite{Lemaitre1985} so that the apparent stiffness can gradually degrade.
...
Unlike metals, concrete does not require ratcheting to be modeled. As a result, only isotropic hardening is conventionally used, with no kinematic hardening. Thus, there is also no need to introduce back stress into the model. The yield function $F$ can be simply expressed as
\begin{gather}
F=\abs{\bar{\sigma}}-\bar{\sigma}_y.
\end{gather}
This states that plastic evolution is triggered whenever the magnitude of the effective stress $\bar{\sigma}$ exceeds the backbone defined by $\bar{\sigma}_y$, which depends on an internal hardening variable.

It becomes apparent that if only one $\bar{\sigma}_y$ is used, there is no way to distinguish tensile behaviour from compressive behaviour. This issue can be addressed by adopting two sets of rules, one for tension and the other for compression.
\begin{gather}
F=\left\{
\begin{array}{ll}
F_t,&\bar{\sigma}>0,\\
...
\end{gather}
This coincides with associative plasticity (the second equality).
\subsubsection{Hardening Law}
Any functions can be chosen as the hardening law, so
\begin{gather}
...
Since damage theory is adopted to account for stiffness and strength degradation, monotonically increasing functions are often chosen as hardening functions.
...
\right.
\end{gather}
In the above expression, $f_t$ and $f_y$ are the crack strength (for tension, positive) and the yield strength (for compression, positive), $h_t$, $h_c$ and $h_d$ are three positive moduli, and $k_0$ denotes the value of $k_c$ at the compressive crush strength. Essentially, $\bar{\sigma}_{y,t}$ is a linear hardening function, while $\bar{\sigma}_{y,c}$ is a bilinear hardening function. The expression can be further simplified by letting $f_c$ denote $f_y+h_ck_0$.
...
\end{gather}
Although the same expression is used for both, note that in a single loading step only one of the two yield functions is activated. Hence, the plastic strain increment $\gamma$ for that step can contribute only to either $k_t$ or $k_c$, not both.
\subsubsection{Degradation}
With the above yield function, flow rule, and hardening law, we have effectively defined a very simple plasticity model with only isotropic hardening, although it may behave differently in tension and compression. It bears some resemblance to the model introduced in \secref{sec:uniaxial_brb}, which adopts the same concept to differentiate tension and compression but uses more complex specific functions.

Similarly, it is desirable to control the degradation rate independently for tension and compression. To this end,
\begin{gather}
d=\left\{
\begin{array}{ll}
d_t,&\bar{\sigma}>0,\\
...
\right.
\end{gather}
The specific forms of tensile damage $d_t$ and compressive damage $d_c$ can vary. For example, the original formulation \cite{Kenawy2020} uses the linear degradation in \cite{Grassl2013}. Here, we use a simpler exponential function.
\begin{gather}
...
For compression, if the original bilinear hardening function \eqsref{eq:k4_hardening} is used, the crush strength should still be reached. Thus, the damage evolution should be delayed. The following function
\begin{gather*}
d_c=1-\exp\left(-\dfrac{\max\left(k_c-k_0,0\right)}{e_{r,c}}\right)
\end{gather*}
can be used so that $d_c$ stays at zero until $k_c$ becomes larger than $k_0$.
\paragraph{Total Energy}
If linear hardening is used, for example,
\begin{gather}
\bar{\sigma}_y=ak+b.
\end{gather}
Here we do not distinguish between tension and compression, and we use the general parameters $a$ and $b$. Combining $\bar{\sigma}_y$ with the damage factor gives the final stress backbone as
\begin{gather}
...
\end{gather}
For this to be smaller than zero (no initial hardening, softening only), we require
\begin{gather}
ac<b.
\end{gather}
If $a=0$, then no matter what value $c$ takes, there is no initial hardening because $b>0$. If $ac\geqslant{}b$, implying $a>0$, the backbone has a peak value at $k=c-\dfrac{b}{a}$.

We can associate $c$ with $b$ by setting $c=\dfrac{b}{\zeta{}E}$, where $\zeta$ is a positive multiplier. The total energy then becomes
\begin{gather}
...
If the reference $\zeta_r$ is given for the reference characteristic length $l_r$, then for any characteristic length $l$, if the total energy needs to remain unchanged,
\begin{gather}
...
Note that here we only enforce the total energy to remain constant. This is not equivalent to obtaining objective results under arbitrary loading magnitudes.
...
To account for this crack-closing mechanism, one more history variable $k_k$ is introduced to allow additional plasticity to develop. The variable $k_k$ tracks crack closing.
...
This mechanism states that, during reloading towards compression ($\bar\sigma<0$ and $\Delta\varepsilon<0$), whenever $k_k<k_t$, that is, crack closing is smaller than crack opening, there is still net crack opening that needs to be closed. Under this condition, the inner plasticity is activated so that part of the strain increment is converted to plastic strain (crack closing). The following two implementation details need to be considered to ensure a correct model.
\paragraph{Entering}
Note that the exact transition step from tensile stress to compressive stress needs special treatment. The tensile stress needs to fully unload to zero; this part involves no plasticity. The remainder is compressive loading, which does involve plasticity. Thus $\Delta\varepsilon^p$ should be limited such that
...
\end{gather}
If the above condition is not satisfied by the plastic strain increment computed from \eqsref{eq:k4_closing_strain}, then we should manually set $\Delta\varepsilon^p=k_{k,n}-k_t$ and update $k_k$ and $\bar{\sigma}$ accordingly.
\paragraph{Remark}
For crack closing, the original model \cite{Kenawy2020} formulates it as a plasticity model but does not use a return-mapping algorithm in the corresponding implementation.
...
To correct the response, however, it has to be defined at least as
\begin{gather}
\bar{\sigma}_k=h_k\left(k_k-k_k^i\right),
\end{gather}
where $k_k^i$ is the initial $k_k$ at the beginning of the current loading path, and needs to be updated whenever load reversal occurs.

We avoid such a complex presentation because it may cause unnecessary confusion. At its core, the crack-closing mechanism tries to close the unclosed crack ($k_t-k_k$) by accumulating a portion of the strain increment into $k_k$ ($\Delta{}k_k=\abs{\Delta\varepsilon}\dfrac{E}{E+h_k}$), and this mechanism is activated only under certain conditions.
...
\end{gather}
The explicit form of $\ddfrac{d}{k}$ should be determined by the damage evolution.
...
It is straightforward to translate the above pseudocode into an implementation.
\begin{cppcode}
...
The crack-closing branch accounts for the entering/exiting details discussed above.
\begin{cppcode}
void NonlinearK4::compute_crack_close_branch() {
    ...
    const auto net_strain = std::fabs(incre_strain(0)) - std::max(0., current_stress(0)) / elastic_modulus;
...
The tension and compression return-mapping algorithms are almost identical, except that they update different history variables and call different functions to compute the backbone and damage. We can therefore unify them and use one implementation for both.
\begin{cppcode}
int NonlinearK4::compute_plasticity() {
    ...
    while(true) {
        ...
        const auto residual = std::fabs(trial_stress(0)) - backbone[0];
...
In computing the damage variables, we can adjust $\zeta$ according to the previous discussion.
\begin{cppcode}
...
pod2 ConcreteK4::compute_tension_damage(const double k) const {
    ...
    const auto factor = std::exp(-k / e_t);
