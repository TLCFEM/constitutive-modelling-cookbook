...
\end{gather}
\subsubsection{Plastic Evolution}
In this work, we still use the first order accurate backward Euler method for numerical integration. In specific, the discretised evolution of plastic strain is written as
\begin{gather}
\bvarepsilon^p_{n+1}=\bvarepsilon^p_{n}+\Delta\bvarepsilon^p=\bvarepsilon^p_{n}+\gamma\bn_{n+1}=\bvarepsilon^p_{n}+\gamma\left(\mb{P}\bsigma_{n+1}+\mb{q}\right).
\end{gather}
...
Some implementations \cite{Krasnovskiy2004,SouzaNeto2008} adopt line search.
...
thus
\begin{gather}
\left(\mb{I}+\gamma{}\mb{E}\mb{P}\right)\bsigma=\bsigma^\text{trial}-\gamma\mb{E}\mb{q}\qquad\longrightarrow\qquad
\bsigma=\left(\mb{I}+\gamma{}\mb{E}\mb{P}\right)^{-1}\left(\bsigma^\text{trial}-\gamma\mb{E}\mb{q}\right).
\end{gather}
...
One can then use root finding methods to find the solution $\gamma$ as the initial guess.

The implementation presented later in this section includes such a logic.
\subsubsection{Consistent Tangent Stiffness}
