...
n_{12}&n_{22}&n_{23}\\
n_{31}&n_{23}&n_{33}
\end{bmatrix}}_{\bn},\qquad\text{expressed in tensor components.}
\end{gather}
...
\end{split}
\end{gather}
Here, $\bn$ is expressed as $\bn=\dfrac{\beeta^\text{trial}}{\norm{\beeta^\text{trial}}}$ due to coaxiality of $\beeta_{n+1}$ and $\beeta^\text{trial}$.
...
&&&&G&\\
&&&&&G
\end{bmatrix}\diag{\mb{c}}\bn=\begin{bmatrix}
2G&&&&&\\
&2G&&&&\\
&&2G&&&\\
&&&2G&&\\
